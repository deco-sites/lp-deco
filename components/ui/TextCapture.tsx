    ...
    <div class="bg-[#FFF] py-16 md:py-24">
      <section class="xl:container mx-auto flex flex-col items-center justify-center gap-8">
        ...
        <div class="flex gap-4">
          { cta?.map((item) => <a class={`text-lg py-4 px-8 font-semibold transition-colors duration-200 ${item?.variant === 'Reverse' ? 'border border-[#181212] bg-[#181212] text-[#FFF] hover:bg-[#FFF] hover:text-[#000]' : 'border border-[#C9CFCF] bg-[#FFF] text-[#000]  hover:bg-[#C9CFCF]'}`}>{item?.text}</a>) }
        </div>
      </section>
    </div>
  )
